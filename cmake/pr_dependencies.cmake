function(pr_set_include_path IDENTIFIER PATH)
    ...
    if(PA_FORCE)
        set(DEPENDENCY_${NORMALIZED_IDENTIFIER}_INCLUDE ${PATH} CACHE PATH "Path to ${PRETTY_IDENTIFIER} include directory." FORCE)
    else()
        set(DEPENDENCY_${NORMALIZED_IDENTIFIER}_INCLUDE ${PATH} CACHE PATH "Path to ${PRETTY_IDENTIFIER} include directory.")
    endif()
endfunction()
...
function(pr_set_library_path IDENTIFIER PATH)
    ...
    pr_get_normalized_identifier_name(${IDENTIFIER})
    if(PA_FORCE)
        set(DEPENDENCY_${NORMALIZED_IDENTIFIER}_LIBRARY ${PATH} CACHE FILEPATH "Path to ${PRETTY_IDENTIFIER} library." FORCE)
    else()
        set(DEPENDENCY_${NORMALIZED_IDENTIFIER}_LIBRARY ${PATH} CACHE FILEPATH "Path to ${PRETTY_IDENTIFIER} library.")
    endif()
endfunction()
...
function(pr_add_include_dir TARGET_NAME IDENTIFIER)
	...
	message("[PR] Adding include directory \"${DEPENDENCY_${NORMALIZED_IDENTIFIER}_INCLUDE}\" (${NORMALIZED_IDENTIFIER}) to target ${TARGET_NAME} with visibility ${VISIBILITY}")
...
function(pr_link_library TARGET_NAME IDENTIFIER)
	...
	message("[PR] Linking library \"${DEPENDENCY_${NORMALIZED_IDENTIFIER}_LIBRARY}\" (${NORMALIZED_IDENTIFIER}) to target ${TARGET_NAME} with visibility ${VISIBILITY}")
